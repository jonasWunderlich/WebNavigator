...
(function() {
  var addClearDiv, addDevInfo, blockSum, block_counter, block_set, blocks, bookMarks, bookmarkIt, createBlocks, createBookmark, createButtons, createHistory, filter, hideInactiveTasks, idToRef, initSlider, lastUrl, lastVid, loadBookmarks, loadHistory, logInfo, max, min, processHistoryItems, processVisitItems, processed, reload, renderItem, renderTaskMenu, shortenTitle, siteHistory, specialise, start, storedBookmarks, storedContexts, tabArray, tabconnections, thelastOne, toggleActiveState, urltoblock, v_max, visitIdAufSID;
  ...
  processVisitItems = function(site, visitItems, callbackFn) {
    ...
    idToRef[id] = referrer;
    if ((type === "link" || type === "form_submit") && (lastVid === ref || lastUrl === site.url.substr(0, 20))) {
      urltoblock[site.url.substr(0, 20)] = block_set;
      null;
    } else {
      if ((type !== "typed" && type !== "keyword" && type !== "generated") && (urltoblock[site.url.substr(0, 20)] != null)) {
  ...
  */
  ...
  renderItem = function(item, divToGo) {
    ...
    panel_div = $("<div>");
    if ((thelastOne.substr(0, thelastOne.length - 3)) === (url.substr(0, url.length - 3))) {
  ...
  createBlocks = function() {
    var $contextgroup, blockdings, contextGroup, item, key, num;
